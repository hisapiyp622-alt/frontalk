...
(function () {
  ...
  function syncForm() {
    ...
    $("h5Kubun").value = state.h5Kubun || "shinki";
    ...
    if (r10gOn) {
      ...
      r10gHint2.innerHTML = (provider() === "@nifty"
  ...
  function qrCardsHtml() {
    ...
    if (state.product === "hikari1g" && state.routerRental !== "nashi" && ROUTER_QR[provider()]) {
      keys.push(ROUTER_QR[provider()]);
    }
    if (isDocomoHikari() && provider() === "@nifty") keys.push("niftyFollow");
    if (state.product === "hikari10g" && state.router10g && num(state.router10gPrice) > 0
  ...
  function renderStaffSheet() {
    ...
    IENAKA_OPTS.forEach(function (o) {
      if (o.for.indexOf(state.product) < 0) return;
      if (o.needsPhone && !phoneOn) return;
      if (o.needsHomeDenwa && !homeDenwaOn()) return;
  ...
  $("h5Kubun").addEventListener("change", function () { state.h5Kubun = this.value; recalc(); });
  ...
  window.__IE_TEST__ = {
    ...
    /* 『開通までの流れ』の文字（お客様にお渡しする紙の中身）。
     * 商材を切り替えたあとに、前の商材のご案内が残っていないかを見る（2026-09-08）。 */
    flowText: function (patch) {
      var keep = JSON.parse(JSON.stringify(state));
      Object.keys(patch || {}).forEach(function (k) { state[k] = patch[k]; });
      applyDefaults();
      var d = document.createElement("div");
      d.innerHTML = flowSheetHtml();
